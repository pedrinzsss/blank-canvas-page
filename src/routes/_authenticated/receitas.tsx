...
function ReceitasPage() {
  ...
  const [type, setType] = useState("Bonificação");
  ...
  const totalRecebidas = transactions
    .filter(t => t.received)
    .reduce((acc, t) => acc + t.amount_cents, 0);

  const total = totalPendentes + totalRecebidas;

  return (
    <AppShell title="Transações" subtitle="Gestão de receitas e entradas">
      <div className="space-y-6 p-4 md:p-6">
        ...
        <div className="rounded-2xl bg-secondary overflow-hidden">
          <div className="grid grid-cols-10 gap-4 px-6 py-4 text-[11px] font-bold uppercase tracking-wider text-muted-foreground border-b border-border/5">
            ...
            <div className="col-span-2">Descrição/Email</div>
          ...
          <div className="divide-y divide-border/5">
            {loading ? (
              ...
              transactions.map((t) => (
                <div key={t.id} className="grid grid-cols-10 gap-4 px-6 py-4 items-center text-sm hover:bg-primary/5 transition-colors">
                  <div className="flex items-center justify-center">
                  ...
                  <div className="col-span-1">
                    <Badge variant={t.received ? "default" : "outline"} className={cn("text-[10px] uppercase", t.received ? "bg-emerald-500" : "text-amber-500 border-amber-500")}>
                      {t.received ? "Recebida" : "Pendente"}
                    </Badge>
                  </div>
                  <div className="col-span-2 flex flex-col">
                    <span className="font-medium truncate text-foreground">Cliente Final</span>
                    <button className="text-[10px] text-primary hover:underline text-left">Ver mais sobre o cliente</button>
                  </div>
                  ...
                  <div className="col-span-2 flex flex-col truncate">
                    ...
                    <span className="text-[10px] text-muted-foreground truncate">cliente@exemplo.com</span>
                  </div>
              ...
              <div className="flex flex-col items-center justify-center py-20 text-center">
                <p className="text-muted-foreground text-sm">Nenhuma receita encontrada para este período.</p>
              </div>
            )}
          </div>
        </div>
      ...
      <Dialog open={isModalOpen} onOpenChange={setIsModalOpen}>
        <DialogContent className="sm:max-w-[500px] bg-card border-border overflow-y-auto max-h-[90vh]">
          <DialogHeader>
            <DialogTitle className="text-xl font-bold">Nova Receita</DialogTitle>
          </DialogHeader>
          ...
          <div className="grid gap-6 py-4">
            ...
            <div className="flex items-center gap-2 bg-secondary/30 p-3 rounded-xl border border-border/50">
              ...
              />
              <Label htmlFor="received" className="text-sm cursor-pointer select-none">Esta receita já foi recebida</Label>
            </div>
