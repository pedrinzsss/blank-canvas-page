          ...
          {(["sandbox", "live"] as Env[]).map((e) => (
            <button
              ...
              className={`rounded-md px-4 py-1.5 text-sm font-medium transition-all ${
                env === e ? "bg-primary text-white shadow" : "text-foreground/70 hover:text-foreground"
              }`}
...
function RevealBanner({
  ...
  const [copied, setCopied] = useState(false);
  return (
    <div className="rounded-2xl border border-amber-500/40 bg-amber-50 dark:bg-amber-500/10 p-5">
      <div className="mb-2 flex items-center justify-between">
        <p className="text-sm font-semibold text-amber-900 dark:text-amber-200">
          {title} · {environment === "sandbox" ? "Sandbox" : "Produção"}
        </p>
        <Button variant="ghost" size="sm" onClick={onClose} className="text-amber-900 dark:text-amber-200 hover:bg-amber-500/10">
          Fechar
        </Button>
      </div>
      ...
      <div className="mt-3 flex items-center gap-2">
        <code className="flex-1 overflow-x-auto rounded-lg border border-amber-500/40 bg-white dark:bg-background/40 p-3 font-mono text-sm text-amber-950 dark:text-amber-100">
          {secret}
        </code>
        <Button
          variant="outline"
          ...
          className="gap-2 border-amber-500/50 bg-transparent text-amber-900 dark:text-amber-100 hover:bg-amber-500/10"
        >
